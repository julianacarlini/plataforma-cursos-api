...
r.get("/", auth, requireRole("ADMIN"), async (req,res,next)=>{
  try{
    ...
    res.json(users.map((u: { id: any; name: any; email: any; role: { name: any; }; blocked: any; })=>({ id:u.id, name:u.name, email:u.email, role:u.role.name, blocked:u.blocked })));
  }catch(e){next(e);}
});
...
r.patch("/:id", auth, requireRole("ADMIN"), validate(userUpdateSchema), async (req,res,next)=>{
  try{
    const id=BigInt(req.params.id); const data:any={...req.body};
    if(data.role){ const role=await prisma.role.findUnique({where:{name:data.role}}); if(role){ data.roleId=role.id; delete data.role; } }
...
r.delete("/:id", auth, requireRole("ADMIN"), async (req,res,next)=>{
  try{ await prisma.user.delete({ where:{ id: BigInt(req.params.id) } }); res.status(204).end(); }
  catch(e){next(e);}
});
...
r.get("/me/profile", auth, async (req,res,next)=>{
  try{
    const me=await prisma.user.findUnique({ where:{ id: BigInt((req as any).user.id) }, include:{role:true}});
    if(!me) throw notFound(); res.json({ id:me.id, name:me.name, email:me.email, role:me.role.name });
  }catch(e){next(e);}
});
