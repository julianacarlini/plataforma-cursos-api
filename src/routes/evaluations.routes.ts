...
r.get("/course/:id", auth, async (req,res,next)=>{
  try{
    const id=Number(req.params.id); const u=(req as any).user;
    const where:any={ courseId:id }; if (u.role==="ALUNO") where.isPublished=true;
...
r.get("/:id", auth, async (req,res,next)=>{
  ...
  if(!ev) return res.status(404).json({message:"Não encontrada"}); res.json(ev);
});
...
r.patch("/:id", auth, requireRole("PROFESSOR"), validate(evaluationPublishSchema), async (req,res,next)=>{
  try{
    ...
    if(!ev) throw new AppError("Avaliação não encontrada",404,"NOT_FOUND");
    if(String(ev.course.teacherId)!==String((req as any).user.id)) throw new AppError("Sem acesso",403,"FORBIDDEN");
...
r.post("/:id/submissions", auth, requireRole("ALUNO"), validate(submissionCreateSchema), async (req,res,next)=>{
  try{
    const ev=await prisma.evaluation.findUnique({ where:{ id:Number(req.params.id) }});
    if(!ev || !ev.isPublished) throw new AppError("Avaliação indisponível",400);
    const meId=BigInt((req as any).user.id);
