...
r.post("/register", validate(registerSchema), async (req, res, next) => {
  try {
    const { name, email, password, role } = req.body as {
      name: string; email: string; password: string; role?: "ALUNO"|"PROFESSOR";
    };

    const roleRow = await prisma.role.findUnique({
      where: { name: (role || "ALUNO") as any }
    });
    if (!roleRow) throw new AppError("Papel inválido", 400);
...
r.post("/login", validate(loginSchema), async (req, res, next) => {
  try {
    ...
    if (user.blocked) throw new AppError("Usuário bloqueado", 401, "UNAUTHORIZED");

    const ok = await bcrypt.compare(password, user.passwordHash);
    if (!ok) throw new AppError("Credenciais inválidas", 401, "UNAUTHORIZED");

    if (!user.role) throw new AppError("Usuário sem papel definido", 500);
...
r.post("/refresh", async (req, res, next) => {
  try {
    const { refreshToken, refresh_token } = req.body as { refreshToken?: string; refresh_token?: string };
    const token = refreshToken || refresh_token;
    if (!token) throw new AppError("Token ausente", 401, "UNAUTHORIZED");

    const data = verifyRefresh(token) as any; // deve conter { id: string }
    if (!data?.id) throw unauthorized("Token inválido");

    const u = await prisma.user.findUnique({
      where: { id: BigInt(data.id) },
      include: { role: true }
    });
    if (!u || u.blocked) throw new AppError("Não autorizado", 401, "UNAUTHORIZED");
...
r.get("/me", auth, async (req, res, next) => {
  try {
    const userId = BigInt((req as any).user.id); // id no JWT é string; aqui convertemos para buscar
    const u = await prisma.user.findUnique({
      where: { id: userId },
      include: { role: { select: { name: true } } }
    });
