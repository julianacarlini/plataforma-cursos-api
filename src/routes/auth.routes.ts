...
r.post("/register", validate(registerSchema), async (req, res, next) => {
  try {
    ...
    const roleRow = await prisma.role.findUnique({ where: { name: (role || "ALUNO") as any } });
...
r.post("/login", validate(loginSchema), async (req, res, next) => {
  try {
    ...
    if (!user || user.blocked) throw new AppError("Credenciais inválidas", 401, "UNAUTHORIZED");
...
r.post("/refresh", async (req, res, next) => {
  try {
    const { refreshToken } = req.body; if (!refreshToken) throw new AppError("Token ausente", 401, "UNAUTHORIZED");
    const data = verifyRefresh(refreshToken) as any;
