...
r.get("/course/:courseId", auth, async (req, res, next) => {
    try {
        ...
        if (me.role === "PROFESSOR") {
            ...
            if (!c || String(c.teacherId) !== String(me.id))
                throw new AppError("Sem acesso a este curso", 403, "FORBIDDEN");
        }
...
r.patch("/:id", auth, requireRole("PROFESSOR"), validate(lessonUpdateSchema), async (req, res, next) => {
    try {
        ...
        if (!l)
            throw new AppError("Aula não encontrada", 404, "NOT_FOUND");
        if (String(l.course.teacherId) !== String(req.user.id))
            throw new AppError("Sem acesso", 403, "FORBIDDEN");
        const up = await prisma.lesson.update({ where: { id }, data: req.body });
        res.json(up);
    }
    catch (e) {
        next(e);
