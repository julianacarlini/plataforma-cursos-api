    ...
    try {
        ...
        if (tag === "recommended") {
            ...
            if (user?.sub) {
                ...
                const cats = Array.from(new Set(enrolled.map((e) => e.course.category)));
                const alreadyIds = new Set(enrolled.map((e) => e.course.id));
                const rows = await prisma.course.findMany({
                    where: {
                        ...where,
...
r.get("/:id", async (req, res, next) => {
    try {
        ...
        const teacher = c.teacher
            ? { id: String(c.teacher.id), name: c.teacher.name }
            : null;
        res.json({ ...c, teacher });
    }
    catch (e) {
...
r.post("/:id/assign", auth, requireRole("PROFESSOR"), async (req, res, next) => {
    try {
        ...
        const teacherCourses = await prisma.course.count({
            where: { teacherId: meId, status: { in: ["PENDING", "APPROVED"] } },
        });
        if (teacherCourses >= 3)
            throw new AppError("Limite de 3 cursos por professor atingido", 400);
        const c = await prisma.course.findUnique({ where: { id } });
        if (!c)
            throw notFound("Curso não encontrado");
        if (c.teacherId)
            throw new AppError("Curso já possui professor", 400);
